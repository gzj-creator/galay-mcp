...
int main() {
    ...
    server.addTool("concat", "Concatenate two strings", concatSchema,
        [](const JsonElement& args) -> std::expected<JsonString, McpError> {
            ...
            if (str1Val.error() || !JsonHelper::GetStringValue(str1Val.value(), str1) ||
                str2Val.error() || !JsonHelper::GetStringValue(str2Val.value(), str2)) {
...
int main() {
    ...
    server.addPrompt("code_review", "Generate code review prompt", reviewArgs,
        ...
        -> std::expected<JsonString, McpError> {
            ...
            if (langVal.error() || !JsonHelper::GetStringValue(langVal.value(), language)) {
            ...
            if (!diffVal.error()) {
                JsonHelper::GetStringValue(diffVal.value(), difficulty);
            }
...
int main() {
    ...
    server.addTool("calculate", "Perform arithmetic operations", calcSchema,
        ...
        -> Coroutine {
            ...
            if (opVal.error() || !JsonHelper::GetStringValue(opVal.value(), op)) {
...
Coroutine clientTask(Runtime& runtime, RunState* state) {
    ...
    if (auto __await_result = co_await client.initialize("http-client", "1.0.0", init_result); !__await_result) {
        // 错误处理：记录日志、重试或提前返回
    }
    if (!init_result) {
    ...
    if (auto __await_result = co_await client.callTool("echo", echoArgs.TakeString(), echo_result); !__await_result) {
        // 错误处理：记录日志、重试或提前返回
    }
    if (echo_result) {
    ...
    if (auto __await_result = co_await client.callTool("calculate", calcArgs.TakeString(), calc_result); !__await_result) {
        // 错误处理：记录日志、重试或提前返回
    }
    if (calc_result) {
...
server.addTool("file_ops", "File operations", fileSchema,
    [](const JsonElement& args) -> std::expected<JsonString, McpError> {
        ...
        if (pathVal.error() || !JsonHelper::GetStringValue(pathVal.value(), path) ||
            opVal.error() || !JsonHelper::GetStringValue(opVal.value(), operation)) {
            return std::unexpected(McpError::invalidParams("Missing parameters"));
        }
        ...
        if (operation == "read") {
            ...
            if (contentVal.error() || !JsonHelper::GetStringValue(contentVal.value(), content)) {
...
server.addPrompt("code_gen", "Generate code generation prompt", codeGenArgs,
    ...
    -> std::expected<JsonString, McpError> {
        ...
        if (langVal.error() || !JsonHelper::GetStringValue(langVal.value(), language) ||
            taskVal.error() || !JsonHelper::GetStringValue(taskVal.value(), task)) {
            return std::unexpected(McpError::invalidParams("Missing parameters"));
        }
        ...
        if (!styleVal.error()) {
            JsonHelper::GetStringValue(styleVal.value(), style);
        }
