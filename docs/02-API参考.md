...
server.addPrompt("write_essay", "Generate essay prompt", args,
    ...
    -> std::expected<JsonString, McpError> {
        ...
        if (topicVal.error() || !JsonHelper::GetStringValue(topicVal.value(), topic)) {
...
kernel::Coroutine run(kernel::Runtime& runtime) {
    ...
    // 连接
    if (auto __await_result = co_await client.connect("http://127.0.0.1:8080/mcp"); !__await_result) {
        // 错误处理：记录日志、重试或提前返回
    }

    // 初始化
    std::expected<void, McpError> init_result;
    if (auto __await_result = co_await client.initialize("my-client", "1.0.0", init_result); !__await_result) {
        // 错误处理：记录日志、重试或提前返回
    }
    ...
    // 断开连接
    if (auto __await_result = co_await client.disconnect(); !__await_result) {
        // 错误处理：记录日志、重试或提前返回
    }
}
```
...
if (auto __await_result = co_await client.listTools(tools_result); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
}
...
if (auto __await_result = co_await client.callTool("echo", argsWriter.TakeString(), call_result); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
}
...
// JSON-RPC 错误码映射
static McpError fromJsonRpcError(int code, const std::string& message,
...
if (doc) {
    ...
    if (JsonHelper::GetObject(root, obj)) {
        ...
        std::string name;
        if (!nameVal.error() && JsonHelper::GetStringValue(nameVal.value(), name)) {
            std::cout << "Name: " << name << '\n';
        }
    }
}
