...
server.addTool("uppercase", "Convert text to uppercase", schema,
    [](const JsonElement& args) -> std::expected<JsonString, McpError> {
        ...
        if (textVal.error() || !JsonHelper::GetStringValue(textVal.value(), text)) {
...
server.addTool("fetch", "Fetch URL content", schema,
    ...
    -> kernel::Coroutine {
        ...
        std::string url;
        auto urlVal = obj["url"];
        if (urlVal.error() || !JsonHelper::GetStringValue(urlVal.value(), url)) {
...
Coroutine callToolExample(McpHttpClient& client) {
    ...
    if (auto __await_result = co_await client.callTool("fetch", argsWriter.TakeString(), result); !__await_result) {
        // 错误处理：记录日志、重试或提前返回
    }
...
public:
    void setupResources(McpStdioServer& server) {
        ...
        server.addResource("config://app", "Config", "App configuration", "application/json",
