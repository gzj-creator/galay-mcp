...
1. 确保资源已注册：
```cpp
server.addResource("file:///data.txt", "Data", "Data file", "text/plain", reader);
```
...
3. 验证资源读取函数：
```cpp
server.addResource("file:///test.txt", "Test", "Test file", "text/plain",
...
1. 分块读取：
```cpp
server.addResource("file:///large.dat", "Large file", "Large data file", "application/octet-stream",
    [](const std::string& uri) -> std::expected<std::string, McpError> {
...
Coroutine myTask() {
    ...
    if (auto __await_result = co_await someOperation(); !__await_result) {
        // 错误处理：记录日志、重试或提前返回
    }
    std::cerr << "After someOperation\n";

    if (auto __await_result = co_await anotherOperation(); !__await_result) {
        // 错误处理：记录日志、重试或提前返回
    }
...
server.addTool("sensitive_tool", "Sensitive operation", schema,
    [&authManager](const JsonElement& args) -> std::expected<JsonString, McpError> {
        ...
        if (!authManager.isAuthorized("current-client")) {
            return std::unexpected(McpError::internalError("Unauthorized"));
        }
...
server.addTool("rate_limited_tool", "Rate limited", schema,
    [&limiter](const JsonElement& args) -> std::expected<JsonString, McpError> {
        if (!limiter.allowRequest("client-id")) {
            return std::unexpected(McpError::internalError("Rate limit exceeded"));
        }
